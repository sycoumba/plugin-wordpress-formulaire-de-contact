...
class WPForms_Views_Shortcode {
	...
	function get_table_column_html( $column_id, $view_settings, $entry ) {
		...
		foreach ( $column_fields as $field_id ) {
			...
			$column_content .= $this->get_field_html( $field_id, $view_settings, $entry_fields );
		}
	...
	function get_field_html( $field_id, $view_settings, $entry_fields ) {
		...
		if ( ! empty( $entry_fields ) && is_array( $entry_fields ) ) {
			...
			} else if ( $form_field_type == 'html' ) {
				//$field_html .= $field_model->get_setting( 'default' );
			}
			$field_value = apply_filters( 'wpforms-views-field-value', $field_value, $view_settings, $sub );
			...
			case 'pagination':
				$field_html .= $this->get_pagination_links( $view_settings, $entry );
				break;
			case 'paginationInfo':
				$field_html .= $this->get_pagination_info( $view_settings, $entry );
				break;
			case 'html':
	...
	function get_pagination_links( $view_settings, $entry ) {
		global $wp;
	...
	function get_pagination_info( $view_settings, $entry ) {
